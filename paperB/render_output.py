...
BASE_LABELS = {
    "A_X_only": "仅 X", "A_A_only": "仅 A", "A_b_only": "仅 b(t)",
    "B_all_core": "三核心", "C_macro": "+宏观", "Layer1_X": "第一层",
    "Layer2_A": "第二层", "Interact_AB": "A×b(t)", "Interact_AX": "A×X",
    "Interact_all": "双交互",
}
BASE_TERMS = {
    "wsdi_days": r"WSDI 天数×0.01 $X_{it}$", "readiness100": r"适应能力 $A_{it}$",
    "b_it": r"当期债务/GDP $b_{it}=debt\_gdp_{it}$", "c_A": r"$A^c_{it}$", "c_X": r"$X^c_{it}$",
...
CRITERION_ORDER = ["theta", "b_it", "mA", "TA", "b_it*mA"]
CRITERION_LABELS = {
    "theta": r"$\widehat\theta^A_{it}$", "b_it": r"$b_{it}$",
    "mA": r"$\widehat m^A_{it}$", "TA": r"$\widehat T^A_{it}$",
    "b_it*mA": r"$b_{it}\widehat m^A_{it}$",
}
...
def render_results() -> str:
    ...
    add(f"- Baseline 全交互模型中，A×b(t) 系数为 {fmt(base_ab['estimate'])}（p={fmt_p(base_ab['p'])}），A×X 系数为 {fmt(base_ax['estimate'])}（p={fmt_p(base_ax['p'])}）。")
    ...
    add("")
    add(r"令 $s_{it}$ 为主权利差比率，$A_{it}$ 为适应能力比率，$X_{it}=wsdi\_days_{it}\times0.01$，$b_{it}=debt\_gdp_{it}$ 为当期债务状态。Baseline 的宏观控制为 Growth、Inflation，不控制 $\ln(ConstantGDP)$；T 指标方程的宏观控制仅为 Inflation，不控制 Growth；Doomloop 的宏观控制仍为 Growth、Inflation。外部控制为 Reserves、Terms of trade。全部模型含国家和年份效应。第 2—3 节使用 `xtlsdvc, initial(bb) bias(2) vcov(50)`：偏差修正精度为 $O((NT)^{-1})$，标准误来自 50 次 bootstrap；动态滞后因变量由 LSDVC 自动加入。第 4 节使用按 `country_id` 聚类的双向固定效应。每个回归使用其因变量、动态滞后项与当前右侧变量的联合非缺失样本，不再设置跨模型或跨阶段固定样本。生成量不向来源回归样本外外推：$\widehat m^A$ 限于 Spread_Interact_all 的实际样本，$\widehat T^A$ 限于 T10_interact_full 的实际样本，theta 限于两个来源样本的交集。`bias(2)` 在代表性规格中与 `bias(1)` 数值接近且稳定；`bias(3)` 曾在当前短而不平衡的面板上产生爆炸性动态系数，因此不作为主规格。")
    add("")
    add("## 2. Baseline：主权利差回归")
    add("")
    add("### 2.1 回归公式")
    add("")
    add(r"$$s_{it}=\alpha_i+\lambda_t+\rho_s s_{i,t-1}+\beta_AA_{it}+\beta_Bb_{it}+\beta_XX_{it}+\beta_{AB}A_{it}b_{it}+\beta_{AX}A_{it}X_{it}+\Gamma_m'W^m_{it}+\varepsilon^m_{it}.$$")
    add("")
    ...
    add("")
    add(model_table(BASE_MODELS[:7], BASE_LABELS, ["wsdi_days", "readiness100", "b_it", "spread_lag", "growth", "inflation_cpi", "reserves", "tt"], BASE_TERMS, base_coefs, base_stats, BASE_FLAGS))
    add("")
    add("**Panel B：交互模型**")
    add("")
...
def render_diagnostics() -> str:
    ...
    add("")
    add("主流程准确对应 workflow：第 2—3 节使用 Blundell–Bond 初始化的动态 LSDVC、`bias(2)` 和 50 次 bootstrap；第 4 节使用一期债务变化、去债务状态控制、readiness 严格一阶差分且不加入滞后状态控制，并固定使用债务全控制 theta cutoff。每个回归按当前因变量、动态滞后项和右侧变量取完整案例；mA_hat 与 TA_hat 分别限制在其首选来源回归的实际样本内，theta 仅在两个来源样本共同覆盖且 b_it 可用时构造。五种阈值判据使用各自的当前变量样本，因此其 RSS 与 Within R² 不作跨判据排名。")
    add("")
    add("### Issues Found")
    add("")
    ...
    add(r"- 债务变化结果为 $\Delta debt_{i,t+1}=F.debt\_gdp_{it}-debt\_gdp_{it}$；理论债务状态统一使用当期 $b_{it}=debt\_gdp_{it}$。")
    ...
    add("")
    selected = {"bond_spreads", "spread_lag", "wsdi_days", "readiness100", "b_it", "growth", "inflation_cpi", "reserves", "tt"}
    ...
    add("")
    add("三个估计阶段均对国家—年份键执行 fail-closed 唯一性检查；不会自动去重。Baseline 与 T 指标的每个动态回归均使用当前因变量、隐含滞后因变量与右侧变量的联合非缺失样本；Doomloop 债务和 readiness 使用各自当前因变量与右侧变量的联合非缺失样本。mA_hat 仅在 Spread_Interact_all 的实际样本内生成，TA_hat 仅在 T10_interact_full 的实际样本内生成，theta 要求两个来源样本共同覆盖且 b_it 可用；五种判据分别使用各自构造量与全控制变量的联合非缺失样本。")
    add("")
    add("### 4.1 独占样本损失")
    add("")
    missing_rows = []
...
def validate_inputs(include_robustness: bool = True) -> None:
    ...
    if len(comparison) != 5 or {row["criterion"] for row in comparison} != set(CRITERION_ORDER):
        raise ValueError("Criterion comparison must contain theta, b_it, mA, TA, and b_it*mA exactly once.")
