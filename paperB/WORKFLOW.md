...
`paperB` 是使用当期债务状态 $b_{it}$ 的主流程。`baseline`、`empirical_theta`、`doomloop` 三个板块的完整 Stata 源码统一保存在 `paperB/code/`；`paperB/run_workflow.ps1` 按依赖顺序直接调用这些源码。所有新结果只写入 `paperB/paperBresult/`，从而与代码、流程文档及项目根目录中的历史快照隔离。

```text
paperB/
├─ run_workflow.ps1          # 当期债务主流程入口
...
- `paperB/code/baseline_twfe.do` → `paperB/paperBresult/baseline/stata_outputs/`
- `paperB/code/empirical_theta.do` → `paperB/paperBresult/empirical_theta/stata_outputs/`
- `paperB/code/doomloop_no_state.do` → `paperB/paperBresult/doomloop/stata_outputs/`
...
两种债务时点的四组补充实验由单独入口联合执行；它要求当期和滞后主流程结果均已存在：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\paperB\run_robustness.ps1
```

该入口依次生成标准化 RSS profile 与近最优 cutoff 区间、固定两套 cutoff 的交叉敏感性、共同 742 个观测的再估计，以及 30 次配对国家区组的全管线 bootstrap。若 30 次 bootstrap 结果已经存在，仅重算其余实验、同步结果并重绘文档可使用：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\paperB\run_robustness.ps1 -SkipBootstrap
```

稳健性产物同步保存在 `paperB/paperBresult/robustness/` 与 `paperB_debt_lag/paperB_debt_lag/robustness/`。30 次外层抽样为控制耗时的诊断性实验；每次重新估计上游利差与税收方程、生成 theta、搜索 cutoff 并估计末阶段分段方程，但外层抽样内使用 `vcov(0)`，不再嵌套 50 次标准误 bootstrap。复制失败会保留状态和原因，汇总以有效复制为分母；只有覆盖缺失或某一规格没有任何有效复制时流程才停止。
...
4. 在 `xtset country_id year` 后构造 `spread_lag=L.bond_spreads`，并定义当期债务状态 `b_it=debt_gdp`；只有严格相邻年份可提供 $s_{i,t-1}$，当期债务不使用滞后算子。
5. 不设置跨规格或跨阶段共同样本；每个 Baseline 回归使用该式因变量、动态滞后因变量与当前右侧变量的联合非缺失观测。
6. 逐步估计仅 X、仅 A、仅 $b_{it}$、三核心、宏观控制、第一层、第二层、A×$b_{it}$、A×X、双交互模型；十个模型均由 LSDVC 自动加入 `L.bond_spreads`，并在机器可读输出中映射为 `spread_lag`。
...
```math
s_{it}=\alpha_i+\lambda_t+\rho_s s_{i,t-1}+\beta_AA_{it}+\beta_Bb_{it}+\beta_XX_{it}
...
6. `mA_hat` 仅在 `Spread_Interact_all` 经 `e(sample)` 与全套必需变量非缺失共同核验的实际估计支持集内构造，`TA_hat` 同理仅在 `T10_interact_full` 的实际估计支持集内构造，不向来源回归样本外外推系数；`theta_hat_A=b_it*mA_hat+TA_hat` 仅在两个来源样本共同覆盖且 `b_it` 非缺失时构造，保存可供 doomloop 直接使用的 panel。
...
最终经验指标统一使用当期债务状态 `b_it=debt_gdp`：
...
第四节只保留一期、去状态变量规格；不再估计另行包含 $b_{it}$ 或 $A_{i,t-1}$ 状态控制的版本。

1. 读取 `empirical_theta_panel.dta`。
2. 在搜索 cutoff 前，先确认 `mA_hat` 来自 `Spread_Interact_all` 的实际样本、`TA_hat` 来自 `T10_interact_full` 的实际样本，再取两个来源样本的交集并重新计算 `b_it*mA_hat+TA_hat`；任一来源样本不覆盖或任一组成项缺失时 theta 必须缺失。
...
Readiness 方程的两支只乘 `FT=interest_revenue`，不乘 $A_{it}$；该方程不含 $A_{i,t-1}$，也不以自身 RSS 选择 cutoff。债务方程不另含 $b_{it}$ 状态控制。两类方程的控制向量均包含 `growth inflation_cpi reserves tt`，不包含 GDP 控制。
...
其中 $\widehat\theta^A_{it}$ 是基准判据，另外四项分别为 `b_it`、`mA_hat`、`TA_hat` 和 `b_it_mA_hat`。执行规则如下：
...
第二类展示经验边际利差节约 $m^A=-\partial Spread/\partial A$ 分别随 `b_it` 与 `wsdi_days` 变化。点位为来源回归样本的 P10、P25、P50、P75、P90；每个面板改变一个调节变量并把另一项固定在 `Interact_all` 来源样本均值。点估计与 95% 置信区间均由该 LSDVC 模型 50 次 bootstrap VCE 的边际效应逐项取负转换，不重新拟合另一模型。对应文件为：
...
5. `b_it` 与当期 `debt_gdp` 逐行一致；
6. 中心化公式与原始尺度公式逐行一致；
7. `mA_hat` 当且仅当观测属于 `Spread_Interact_all` 的实际样本，`TA_hat` 当且仅当观测属于 `T10_interact_full` 的实际样本，`theta_hat_A=b_it*mA_hat+TA_hat` 当且仅当两个来源样本共同覆盖且 `b_it` 可用；
...
`run_robustness.ps1` 已完成两种债务时点的标准化 RSS、交叉 cutoff、共同 742 末阶段样本和 30 次国家区组全管线 bootstrap，并把结果写入两份自动生成的 `paperB_results.md`。共同 742 比较固定末阶段 country-year，但沿用各自主流程在自然样本上生成的 theta，未在 742 样本上重估上游方程。bootstrap 的每个复制则重新估计 baseline、tax、theta、cutoff 和最终 kink 方程，且两种规格使用同一组国家抽样编号，便于配对比较。

当前证据仍是固定效应相关性证据，不构成因果识别。30 次复制只适合报告 cutoff 分布和分支符号稳定率，不足以形成精确尾部概率或正式置信区间。正式推断应把外层复制提高到至少 200 次，并进一步检查不同 trimming、候选网格和样本窗口；主表标准误仍应与全管线不确定性明确区分。
