...
> 数据：`data0804/invest_panel_weo.csv` 与 `WSDI/data/processed/wsdi_sovereign61_1995_2018.csv`；整合生成时间：2026-08-30 14:00（Asia/Shanghai）。
...
- 第四节唯一主规格的债务 cutoff 为 -0.0154；债务两支联合检验 p=<0.001，使用同一 cutoff 的 readiness 两支联合检验 p=0.015。
- 五个替代判据按各自当前变量取完整案例，N 范围为 744–1,005；完整 theta 的样本内 RSS=1.143181。因样本量可能不同，RSS 不作跨判据排名。
...
债务方程不另加入 $b_{it}$ 状态项，readiness 方程不加入 $A_{i,t-1}$。$\widehat c_B^\theta$ 仅由债务全控制方程在 theta 的 P10—P90 观测值中按最小 RSS 选择；readiness 不进行独立 cutoff 搜索。两类方程均显式控制 $X_{it}$，并依次加入 Growth、Inflation、Reserves 与 Terms of trade。

### 4.2 债务变化方程

| 变量/统计量 | (DN1_core) 核心项 | (DN2_macro) +宏观 | (DN3_full) +全控制 |
| --- | ---: | ---: | ---: |
| $A_{it}(c-\widehat\theta^A_{it})_+$ | -0.684<br>(-0.523) | -0.2442<br>(-0.209) | -0.076<br>(-0.065) |
| $A_{it}(\widehat\theta^A_{it}-c)_+$ | -0.8511***<br>(-4.233) | -0.9597***<br>(-5.076) | -0.9303***<br>(-5.294) |
| WSDI 天数×0.01 $X_{it}$ | 0.0724***<br>(2.885) | 0.0786***<br>(3.199) | 0.0783***<br>(3.073) |
| Growth | — | -0.6022***<br>(-6.13) | -0.6067***<br>(-6.338) |
| Inflation | — | -0.1509<br>(-0.948) | -0.1272<br>(-0.791) |
| Reserves | — | — | -0.0344<br>(-0.659) |
| Terms of trade | — | — | 0.038<br>(1.026) |
| 宏观控制 | 否 | 是 | 是 |
| 外部控制 | 否 | 否 | 是 |
| 国家固定效应 | 是 | 是 | 是 |
| 年份固定效应 | 是 | 是 | 是 |
...
| 年份数 | 20 | 20 | 20 |
| 样本量 | 744 | 744 | 744 |
| Within $R^2$ | 0.276 | 0.336 | 0.342 |
| Overall $R^2$ | 0.077 | 0.121 | 0.135 |

### 4.3 Readiness 一阶差分方程：固定使用债务 cutoff

| 变量/统计量 | (RDN1_core) 核心项 | (RDN2_macro) +宏观 | (RDN3_full) +全控制 |
| --- | ---: | ---: | ---: |
| $FT_{it}(\widehat c_B^\theta-\widehat\theta^A_{it})_+$ | -2.2797<br>(-1.294) | -3.2737<br>(-1.644) | -3.9601*<br>(-1.727) |
| $FT_{it}(\widehat\theta^A_{it}-\widehat c_B^\theta)_+$ | 0.4492<br>(1.512) | 0.6624**<br>(2.64) | 0.6511**<br>(2.605) |
| WSDI 天数×0.01 $X_{it}$ | 0.007<br>(1.028) | 0.0057<br>(0.812) | 0.0055<br>(0.786) |
| Growth | — | 0.0889***<br>(3.355) | 0.0897***<br>(3.302) |
| Inflation | — | -0.0535<br>(-1.579) | -0.0566*<br>(-1.754) |
| Reserves | — | — | 0.004<br>(0.484) |
| Terms of trade | — | — | -0.0048<br>(-0.597) |
| 宏观控制 | 否 | 是 | 是 |
| 外部控制 | 否 | 否 | 是 |
| 国家固定效应 | 是 | 是 | 是 |
| 年份固定效应 | 是 | 是 | 是 |
...
| 年份数 | 20 | 20 | 20 |
| 样本量 | 744 | 744 | 744 |
| Within $R^2$ | 0.199 | 0.211 | 0.211 |
| Overall $R^2$ | 0.174 | 0.178 | 0.177 |

### 4.4 全控制结果、边际效应与图形

| 结果方程 | cutoff 来源 | cutoff | RSS | 候选数 | N_low | N_high | 低支系数 | p_L | 高支系数 | p_H |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 债务变化 | 债务全控制 RSS | -0.0154 | 1.143181 | 594 | 144 | 600 | -0.076 | 0.949 | -0.9303 | <0.001 |
| Readiness | 继承债务 cutoff | -0.0154 | 0.197346 | — | — | — | -3.9601 | 0.090 | 0.6511 | 0.012 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 债务变化 | P10 | -0.0219 | -0.0005 | 0.0077 | 0.949 | [-0.016, 0.015] |
| 债务变化 | P25 | -0.0132 | -0.002 | 0.0004 | <0.001 | [-0.0028, -0.0013] |
| 债务变化 | P50 | 0.0028 | -0.0169 | 0.0032 | <0.001 | [-0.0233, -0.0105] |
| 债务变化 | Mean | 0.017 | -0.0301 | 0.0057 | <0.001 | [-0.0415, -0.0187] |
| 债务变化 | Cutoff | -0.0154 | 0 | 0 | — | [0, 0] |
| 债务变化 | P75 | 0.0263 | -0.0387 | 0.0073 | <0.001 | [-0.0535, -0.024] |
| 债务变化 | P90 | 0.0665 | -0.0762 | 0.0144 | <0.001 | [-0.1051, -0.0473] |
| Readiness（债务 cutoff） | P10 | -0.0219 | -0.026 | 0.0151 | 0.090 | [-0.0563, 0.0042] |
| Readiness（债务 cutoff） | P25 | -0.0132 | 0.0014 | 0.0005 | 0.012 | [0.0003, 0.0025] |
| Readiness（债务 cutoff） | P50 | 0.0028 | 0.0118 | 0.0045 | 0.012 | [0.0027, 0.0209] |
| Readiness（债务 cutoff） | Mean | 0.017 | 0.0211 | 0.0081 | 0.012 | [0.0048, 0.0373] |
| Readiness（债务 cutoff） | Cutoff | -0.0154 | 0 | 0 | — | [0, 0] |
| Readiness（债务 cutoff） | P75 | 0.0263 | 0.0271 | 0.0104 | 0.012 | [0.0062, 0.048] |
| Readiness（债务 cutoff） | P90 | 0.0665 | 0.0533 | 0.0205 | 0.012 | [0.0122, 0.0945] |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| $\widehat\theta^A_{it}$ | -0.0154 | -0.076 | 0.949 | -0.9303 | <0.001 | Partial (-,-) | 1.143181 | 0.3417 | 744 | 144 | 600 |
| $b_{it}$ | 0.3082 | 0.2156 | 0.120 | -0.1469 | <0.001 | Match (+,-) | 1.606572 | 0.2976 | 1,005 | 221 | 784 |
| $\widehat m^A_{it}$ | 0.0034 | 0.2287 | 0.902 | -1.7812 | <0.001 | Match (+,-) | 1.132527 | 0.3479 | 744 | 75 | 669 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 当期债务 | 0.1% | -0.0154 | [-0.0219, 0.0004] | [-0.474, -0.046] | 273/594 | 46.0% |
| 当期债务 | 0.5% | -0.0154 | [-0.0219, 0.0663] | [-0.474, 1.221] | 546/594 | 91.9% |
| 当期债务 | 1% | -0.0154 | [-0.0219, 0.0663] | [-0.474, 1.221] | 594/594 | 100.0% |
| 滞后一期债务 | 0.1% | 0.0518 | [0.0505, 0.0518] | [1.247, 1.281] | 4/592 | 0.7% |
| 滞后一期债务 | 0.5% | 0.0518 | [0.0452, 0.0518] | [1.108, 1.281] | 18/592 | 3.0% |
| 滞后一期债务 | 1% | 0.0518 | [0.0353, 0.0518] | [0.847, 1.281] | 50/592 | 8.4% |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 当期债务 | 当期债务 | -0.0154 | -0.076 | 0.949 | -0.9303 | <0.001 | 1.143181 | 744 | 144 | 600 |
| 当期债务 | 滞后一期债务 | 0.0518 | 1.3081 | <0.001 | -0.6735 | <0.001 | 1.146197 | 744 | 642 | 102 |
| 滞后一期债务 | 当期债务 | -0.0154 | 1.514 | 0.313 | -1.7118 | <0.001 | 1.083933 | 742 | 123 | 619 |
| 滞后一期债务 | 滞后一期债务 | 0.0518 | 2.5496 | <0.001 | -0.9722 | <0.001 | 1.068936 | 742 | 666 | 76 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 当期债务 | -0.0154 | -0.1009 | 0.932 | -0.9375 | <0.001 | 1.142153 | 742 | 143 | 599 |
| 滞后一期债务 | 0.0523 | 2.5495 | <0.001 | -0.9617 | <0.001 | 1.06862 | 742 | 667 | 75 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 当期债务 | 30/30 | -0.0802 | -0.0201 | 0.0148 | 0.059 | 0.1866 | 73.3% | 100.0% | 73.3% |
| 滞后一期债务 | 30/30 | -0.0478 | -0.0113 | 0.0228 | 0.0824 | 0.2052 | 86.7% | 96.7% | 86.7% |

两种规格使用 seed=20260830 的同一国家抽样序列；有效/请求复制为 当期债务 30/30、滞后一期债务 30/30，合计失败 0 个规格复制。每次重新估计两条 theta 来源方程、theta、cutoff 与最终债务方程，但不在外层抽样内再嵌套 50 次 LSDVC VCE。cutoff 分布很宽，说明单一点 cutoff 的定位不稳定；本实验仅是小规模稳定性诊断，不是正式置信区间。
