...
local core     vulnerability_delta100 readiness_delta100 debt_gdp
...
foreach v in readiness_delta100 debt_gdp vulnerability_delta100 {
...
label variable c_b "Mean-centered debt/GDP ratio"
...
local m3  "A_b_only"
local r3  "debt_gdp `always'"
local q3  "s_it = alpha_i + lambda_t + beta_B b_it + epsilon_it"
local m4  "B_all_core"
local r4  "vulnerability_delta100 readiness_delta100 debt_gdp `always'"
local q4  "s_it = alpha_i + lambda_t + beta_X X_it + beta_A A_it + beta_B b_it + epsilon_it"
local m5  "C_macro"
local r5  "vulnerability_delta100 readiness_delta100 debt_gdp `always' inflation_cpi"
local q5  "s_it = alpha_i + lambda_t + beta_X X_it + beta_A A_it + beta_B b_it + Gamma_macro W_it + epsilon_it"
local m6  "Layer1_X"
local r6  "vulnerability_delta100 debt_gdp `always' inflation_cpi reserves tt"
local q6  "s_it = alpha_i + lambda_t + beta_X X_it + beta_B b_it + Gamma_Xs W_it + epsilon_it"
local m7  "Layer2_A"
local r7  "vulnerability_delta100 readiness_delta100 debt_gdp `always' inflation_cpi reserves tt"
...
foreach mid in C_macro Layer1_X Layer2_A {
    ...
    foreach pair in "vulnerability_delta100 base_X" "readiness_delta100 base_A" "debt_gdp base_b" {
...
foreach spec in "Interact_AB debt_gdp int_AB" "Interact_AX vulnerability_delta100 int_AX" {
...
estimates restore Interact_all
foreach spec in "debt_gdp int_AB" "vulnerability_delta100 int_AX" {
...
foreach mid in Layer2_A Interact_all {
    if "`mid'"=="Layer2_A" local vrhs "vulnerability_delta100 readiness_delta100 debt_gdp `always' inflation_cpi reserves tt"
...
preserve
    keep country_name iso3 country_id year ConstantGDP debt_gdp growth ln_constantgdp sample_common duplicate_key
