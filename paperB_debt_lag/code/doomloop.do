...
foreach v in debt_gdp b_pre_theta mA_hat spread_saving_component TA_hat theta_hat_A {
...
generate double b_pre_mapping_diff = abs(b_pre_theta-debt_gdp) if !missing(b_pre_theta,debt_gdp)
...
scalar max_b_pre_mapping_diff = r(max)
quietly count if b_pre_theta!=debt_gdp
scalar bad_b_pre_mapping_rows = r(N)
if scalar(max_theta_reconstruction_diff)>1e-10 | scalar(max_b_pre_mapping_diff)>1e-10 | scalar(bad_b_pre_mapping_rows)>0 {
    display as error "Theta input is not debt_gdp*mA_hat + TA_hat, or b_pre is not debt_gdp."
    log close mainlog
    exit 459
}
...
label variable b_pre_mapping_diff "Absolute difference between b_pre alias and debt_gdp"
...
post `p_formula' ("b_pre maps exactly to debt_gdp") (scalar(max_b_pre_mapping_diff)) (1e-10) (scalar(max_b_pre_mapping_diff)<=1e-10 & scalar(bad_b_pre_mapping_rows)==0)
...
post `p_meta' ("bad_b_pre_debt_gdp_mapping_rows") (scalar(bad_b_pre_mapping_rows))
