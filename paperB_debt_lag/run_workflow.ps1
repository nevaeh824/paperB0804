...
$resultRoot = Join-Path $paperRoot 'paperB_debt_lag'
...
foreach ($model in $baselineRawBModels) {
    if (@($baselineCoefficientRows | Where-Object { $_.model -eq $model -and $_.variable -eq 'b_pre' }).Count -ne 1 -or @($baselineCoefficientRows | Where-Object { $_.model -eq $model -and $_.variable -eq 'b_it' }).Count -gt 0) {
        throw "Baseline model $model must use lagged b_pre rather than contemporaneous b_it."
    }
}
