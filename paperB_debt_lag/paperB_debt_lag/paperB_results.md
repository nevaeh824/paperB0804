...
| 变量/统计量 | (A_X_only) 仅 X | (A_A_only) 仅 A | (A_b_only) 仅 b(t-1) | (B_all_core) 三核心 | (C_macro) +宏观 | (Layer1_X) 第一层 | (Layer2_A) 第二层 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
| 滞后一期债务/GDP $b_{i,t-1}=debt\_gdp_{i,t-1}$ | — | — | 0.0004<br>(0.143) | 0.0016<br>(0.457) | 0.0056*<br>(1.837) | 0.0042<br>(0.916) | 0.0046<br>(1.043) |
...
债务方程不另加入 $b_{i,t-1}$ 状态项，readiness 方程不加入 $A_{i,t-1}$。$\widehat c_B^\theta$ 仅由债务全控制方程在 theta 的 P10—P90 观测值中按最小 RSS 选择；readiness 不进行独立 cutoff 搜索。两类方程均显式控制 $X_{it}$，并依次加入 Growth、Inflation、Reserves 与 Terms of trade。
...
| $b_{i,t-1}$ | 0.211 | 0.0735 | 0.658 | -0.2005 | <0.001 | Match (+,-) | 1.35845 | 0.3634 | 984 | 99 | 885 |
...
| $b_{i,t-1}\widehat m^A_{it}$ | 0.036 | 3.2475 | <0.001 | -1.3756 | 0.002 | Match (+,-) | 1.075131 | 0.3809 | 742 | 573 | 169 |
