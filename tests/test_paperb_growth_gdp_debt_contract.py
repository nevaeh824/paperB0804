...
class PaperBGrowthGdpDebtContractTests(unittest.TestCase):
    def assert_models_have_always_controls(self, rows, models):
        for model in models:
            with self.subTest(model=model):
                variables = {row["variable"] for row in rows if row["model"] == model}
                self.assertTrue({"growth", "ln_capitagdp"}.issubset(variables))

    def test_every_reported_regression_has_growth_and_capita_gdp_controls(self):
        ...
        self.assert_models_have_always_controls(baseline, baseline_models)
        ...
        self.assert_models_have_always_controls(theta, theta_models)
        ...
        self.assert_models_have_always_controls(doom, doom_models)
    ...
    def test_empirical_theta_uses_current_gdp_and_log_debt(self):
        ...
        required = {
            "CurrentGDP", "debt", "ln_currentgdp", "ln_debt", "Y_outcome", "Y_lag",
            "YA_hat", "ln_debt_mA_hat", "theta_hat_A",
        }
        self.assertTrue(required.issubset(panel[0]))
        for row in panel:
            current_gdp = as_float(row["CurrentGDP"])
            debt = as_float(row["debt"])
            ln_currentgdp = as_float(row["ln_currentgdp"])
            ln_debt = as_float(row["ln_debt"])
            if current_gdp is not None:
                self.assertAlmostEqual(ln_currentgdp, math.log(current_gdp), places=7)
            if debt is not None:
